...
| auth0logs | microservices | あり (`nexuscrm-auth0logs.a0japan.solutions`)。Auth0 Log Stream の Webhook 受信専用。`role: microservices` を指定することで既存の `microservices-sg-policy`（frontend/mcp からの inbound を既に許可済み）を再利用し、Pod 単位の SecurityGroupPolicy を新規に作らずに済む |
